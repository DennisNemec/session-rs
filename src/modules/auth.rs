...
impl IntoResponse for AuthError {
    fn into_response(self) -> axum::response::Response {
        match self {
            AuthError::Tokio(_) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Tokio error").into_response()
            }
            AuthError::Json(_) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Invalid format").into_response()
            }
            ...
            AuthError::InvalidCookie => {
                (StatusCode::BAD_REQUEST, "Invalid session cookie").into_response()
            }
            AuthError::InvalidCsrfToken => {
